...
class Animal:
	...
	def mutate(self):
		...
		if random.random() < 0.5:
			self.genes[random.randint(0,2)][random.randint(0,1)] = random.randint(0,2)
			print self.genes
...
class World:
	...
	def kill(self):
		...
		for animal in self.animals:
			kill = 0
			foodFitness = abs(min(self.food - animal.genes[0][0], self.food - animal.genes[0][1]))
			predFitness = abs(min(self.pred - animal.genes[1][0], self.pred - animal.genes[1][1]))
